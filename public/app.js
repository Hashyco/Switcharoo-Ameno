...
function modeAwardCard(a){
  const mode=a?.mode||"";
  let detail="";
  if(a?.id&&mode==="Hardpoint")detail=`${Number(a.kills_per_map||0).toFixed(2)} bajas/mapa · ${a.hill_time||0}s Hill · ${a.objective_kills||0} Obj. Kills`;
  if(a?.id&&mode==="Overload")detail=`${Number(a.kills_per_map||0).toFixed(2)} bajas/mapa · ${a.overloads||0} Overloads · ${a.kill_overloads||0} Kill OL`;
  if(a?.id&&mode==="Search & Destroy")detail=`${Number(a.kills_per_map||0).toFixed(2)} bajas/mapa · ${a.plants||0} Plants · ${a.defuses||0} Defuses`;
...
function playerStatsTable(rows,compact=false){
  return `<div class="table-wrap"><table><thead><tr><th>Pos.</th><th>Jugador</th><th>Equipo</th><th>Rating</th><th>K/D</th><th>Objective Score</th><th>Bajas/Mapa</th><th>Asist./Mapa</th><th>Mapas</th><th>Bajas</th><th>Muertes</th><th>Asist.</th><th>Obj. Kills</th><th>Hill</th><th>Overloads</th><th>Kill OL</th><th>Plants</th><th>Defuses</th><th>Carrier Kills</th></tr></thead>
    <tbody>${rows.map((p,i)=>`<tr><td><span class="rank">${i+1}</span></td><td><strong>${esc(p.name)}</strong></td><td>${esc(p.team_name)}</td><td><strong>${rating(p.performance_rating)}</strong></td><td>${Number(p.kd).toFixed(2)}</td><td>${points(p.objective_score)}</td><td>${Number(p.kills_per_map||0).toFixed(2)}</td><td>${Number(p.assists_per_map||0).toFixed(2)}</td><td>${p.maps}</td><td>${p.kills}</td><td>${p.deaths}</td><td>${p.assists}</td><td>${p.objective_kills}</td><td>${p.hill_time}s</td><td>${p.overloads}</td><td>${p.kill_overloads}</td><td>${p.plants}</td><td>${p.defuses}</td><td>${p.bomb_carrier_kills}</td></tr>`).join("")}</tbody></table></div>`;
}
...
function statsPage(){
  ...
  return head("Estadísticas","Ranking automático mediante un rating ponderado y normalizado de 0 a 100.")+`<section class="score-formula card card-body"><h3>Fórmula del rating de jugadores</h3><p class="muted">Cada componente se normaliza frente al mejor valor del mismo ranking. El resultado final ordena todas las posiciones y determina automáticamente los MVP.</p><div class="formula-grid"><span>K/D normalizado <b>35%</b></span><span>Objective Score normalizado <b>25%</b></span><span>Bajas por mapa normalizadas <b>30%</b></span><span>Asistencias por mapa normalizadas <b>10%</b></span></div><h4>Composición del Objective Score</h4><div class="formula-grid"><span>Overload <b>+300</b></span><span>Kill Overload <b>+125</b></span><span>Plant <b>+100</b></span><span>Defuse <b>+100</b></span><span>Objective Kill <b>+125</b></span><span>Cada 5 s en Hill <b>+15</b></span></div></section>
