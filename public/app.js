const S={state:null,token:localStorage.saToken||"",user:JSON.parse(localStorage.saUser||"null"),page:location.hash.slice(1)||"home"};
...
async function openPickBan(id){
  ...
  if(!action){toast("Pick & Ban completado","success");render();return}
  const used=m.pickban.filter(x=>x.mode===action.mode&&x.map_name).map(x=>x.map_name);
  const pools={"Hardpoint":["Sake","Colossus","Den","Scar","Gridlock","Hacienda"],"Search & Destroy":["Den","Gridlock","Raid","Fringe","Sake","Hacienda"],"Overload":["Den","Scar","Gridlock","Exposure"]};
  const available=pools[action.mode].filter(x=>!used.includes(x));
  const currentTeam=team(action.team_id);
  if(S.user?.role!=="admin"&&S.user?.teamId!==action.team_id){
    toast(`Ahora es el turno de ${currentTeam.name}`,"info");
    return;
  }
  const numbered=available.map((x,i)=>`${i+1}. ${x}`).join("\n");
  const choice=prompt(`Turno de ${currentTeam.name}\n${action.action_type.toUpperCase()} · ${action.mode}\n\nEscribe el número o el nombre del mapa:\n${numbered}`);
  if(!choice)return;
  const n=Number(choice.trim());
  const selected=Number.isInteger(n)&&n>=1&&n<=available.length?available[n-1]:choice.trim();
  try{
